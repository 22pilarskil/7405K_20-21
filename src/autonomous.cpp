...
void autonomous()
{
	...
	//intake format: index 0, 1 is the intakes, 2 is the indexer, 3 is the fly

    //booleans to pass balls for the listed balls
    //format: t1_2 is towers 1 through 2
    bool t1_2 = false;
    bool t3_4 = false;
    bool t5_9 = false;
	...
	Robot::intake({0, 0, 127, 0}); //index up
	Robot::move_to({700, 5720, 269}); //tower 5
	...
	Robot::move_to({830, 5350, 435}, {1, 1, 1}, {3, 3, 1}); //turning
    Robot::intake({0, 0, 127, -90}); //pooping
	...
	//first ball
	Robot::move_to({1480, 3690, 435}); //ball point
	...
	//tower
	Robot::move_to({2420, 4120, 313}); //tower 6
	...
	Robot::balls_intake_toggle(600, 0);
	Robot::start_task("OUTTAKE7", Robot::balls_intake);
	Robot::move_to({2160, 3860, 312}); //backout
	Robot::intake({0, 0, 127, -127}); //pooping

	//first ball
	Robot::move_to({3070, 3270, 401}); //ball point
	Robot::kill_task("OUTTAKE7");
	Robot::intake({127, 127, 127, 0}); //intake and indexer is continuous for next ball
	...
	Robot::intake({0, 0, 127, 0}); //intake shut off
    ...
    Robot::intake({0, 0, 127, -127}); //pooping
	Robot::move_to({3900, 2670, 488}); //turning
	...
	Robot::balls_intake_toggle(600, 0);
	Robot::start_task("OUTTAKE8", Robot::balls_intake); //ratchet for backout
	Robot::move_to({1520, 1970, 586}); //backout
}